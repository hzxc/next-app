...
const FeeData: NextPageWithLayout = () => {
  ...
  const { data, isError, isLoading } = useFeeData({ chainId: 56 });
  ...
  return (
    <div className='w-[460px] mx-auto mt-4'>
      ...
      <div>
        <p>
          {isLoading ? 'Fetching fee data…' : ''}
          {isError ? 'Error fetching fee data' : ''}
        </p>
        <p className='break-all'>Fee data: {JSON.stringify(data?.formatted)}</p>
      </div>
    </div>
  );
};

FeeData.getLayout = function getLayout(page: ReactElement) {
